<%-- 
    Document   : artedit
    Created on : 1 Nov, 2017, 8:43:09 PM
    Author     : brunda
--%>
...
.col_280 { width: 280px; }
    ...
    <body>
        <form method="get" action="updateart.jsp">
       <br><br>
<table border="1">
<tr><th>Art ID</th><th>Artist</th><th>Image</th><th>Name</th><th>Type</th><th>Size</th><th>Cost</th><th>Number of copies</th><th>Date upload</th><th>Frame</th><th>Discount</th><th>Description</th></tr>
<%  String id=request.getParameter("id");
    Class.forName("com.mysql.jdbc.Driver").newInstance();
        java.sql.Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/trial","root","root");
        Statement st=con.createStatement();
        ResultSet rs=st.executeQuery("select * from artwork where art_id='"+id+"'");
        while(rs.next())
        {
        %>
<tr>
<td><input type="text" name="artid" value="<%=rs.getString(1)%>"></td>
<td><input type="text" name="aid" value="<%=rs.getString(2)%>"></td>
<td><input type="text" name="img" value="<%=rs.getString(3)%>"></td>
<td><input type="text" name="name" value="<%=rs.getString(4)%>"></td>
<td><input type="text" name="type" value="<%=rs.getString(5)%>"></td>
<td><input type="text" name="size" value="<%=rs.getString(6)%>"></td>
<td><input type="text" name="cost" value="<%=rs.getString(7)%>"></td>
<td><input type="text" name="noc" value="<%=rs.getString(8)%>"></td>
<td><input type="text" name="du" value="<%=rs.getString(9)%>"></td>
<td><input type="text" name="fid" value="<%=rs.getString(10)%>"></td>
<td><input type="text" name="dis" value="<%=rs.getString(11)%>"></td>
